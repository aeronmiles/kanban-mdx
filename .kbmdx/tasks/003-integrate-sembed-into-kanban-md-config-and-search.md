...
Wire the sembed library (submodule at sembed/) into kanban-mdx as an opt-in semantic search feature. Requires config schema changes, migration, and integration into both CLI and TUI search paths.
...
- [ ] Add `kbmdx embed sync` subcommand — force full re-index of all tasks
- [ ] Add `kbmdx embed status` subcommand — show index stats (doc count, stale count, last sync)
- [ ] Add `kbmdx embed clear` subcommand — delete embeddings.json
...
- [ ] `kbmdx list --semantic "query"` works end-to-end with Voyage AI
