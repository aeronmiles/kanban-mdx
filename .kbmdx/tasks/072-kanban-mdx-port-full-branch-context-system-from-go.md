...
kbmdx has branch assignment (b/W keys) and worktree board filter (w toggle), but is missing the entire **context model** that makes Go kanban-mdx's branch/worktree features a workflow tool rather than just metadata bookkeeping.
