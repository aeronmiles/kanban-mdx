...
Make `kanban-md` commands work from any git worktree by automatically resolving the board in the main worktree. Currently, `FindDir()` fails in worktrees because `kanban/` is `.gitignore`-d and never present. This forces users/agents back to board home for every board mutation.
...
- Add a `kanban-md which` command that prints the current task based on branch detection

### Phase 3: `switch`/`park` workflow commands (follow-up)

Automate the worktree lifecycle:
- `kanban-md switch <ID>`: pick + claim + `git worktree add` + print shell `cd` command
- `kanban-md park [<ID>]`: merge to main + move done + `git worktree remove`
