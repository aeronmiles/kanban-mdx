...
// Re-export render helpers so app.rs can reach them via super::render::*.
pub(crate) use detail::build_detail_lines;
pub(crate) use detail::build_file_lines;
...
use overlays::{render_block_reason, render_debug, render_help, render_search_help};
...
pub fn render(app: &mut App, frame: &mut Frame) {
    ...
    match app.view {
        ...
        AppView::Guide => {
            guide::render_guide(app, frame);
        }
        AppView::BlockReason => {
            // Render background view, then overlay.
            match app.block_return_view {
                AppView::Detail => render_detail(app, frame),
                _ => render_board(app, frame),
            }
            render_block_reason(app, frame);
        }
        AppView::FilePicker => {
            // Render the background view the picker was opened from.
            match app.file_picker.return_view {
                AppView::Detail => render_detail(app, frame),
                AppView::Guide => guide::render_guide(app, frame),
                _ => render_board(app, frame),
            }
            pickers::render_file_picker(app, frame);
        }
    }
